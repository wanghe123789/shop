...
<header id="pc-header">
	<div class="pc-header-nav">
		<div class="pc-header-con">
			<div class="fl pc-header-link" >您好！，欢迎来云购物 <a href="index.php?r=user/login" target="_blank">请登录</a> <a href="index.php?r=user/register" target="_blank"> 免费注册</a></div>
...
<div class="Xcontent">
	<ul class="Xcontent01">
		...
		<form action="index.php?r=order/order" method="post">
		<ol class="Xcontent13 clearfix">

			<div class="Xcontent14 clearfix"><a href="#"><p><?php echo $one_good['g_name']?></p></a></div>
			...
			<div class="Xcontent17">
				...
				<p class="Xcontent19" >￥<span id="sku_price">0</span>元</p>
				<div class="Xcontent20">
					<p class="Xcontent21">库存</p>
					 <p class="Xcontent19">有<span id="stock">0</span>件商品</p>
				</div>
		...
		<?php foreach ($arr1 as $k => $v){ ?>
			<div class="Xcontent26">
				<input type="hidden"  class="attr_name" value="<?php echo $v[0] ?>">
				<span><?php  $name = Yii::$app->db->createCommand("SELECT attr_name FROM attr WHERE attr_id=".$k)->queryAll();echo $name[0]['attr_name'];?>
				</span>
				 <?php foreach ($v as $ke => $va) {  ?>
						<input type="checkbox" name="attr_val" class="attr_val" value="<?php echo $va;?>">
						    <?php $attr_name = Yii::$app->db->createCommand("SELECT attr_val_name FROM attr_val WHERE att_val_id=".$va)->queryone();echo $attr_name['attr_val_name'];?>
				<?php } ?>
			</div>
         <?php } ?>
          <input type="hidden" name="hidden_sku" id="hidden_sku" value="">
			<div class="Xcontent30">
				<p class="Xcontent31">购买数量</p>
				<div class="Xcontent32"><img src="images/shangpinxiangqing/X15.png"></div>
				<form>
					<input class="input" name="number" value="1" id="number">
				</form>
				<div class="Xcontent33"><img src="images/shangpinxiangqing/16.png"></div>
			</div>
			<input type="hidden" id="g_id" name="g_id" value="<?php echo $g_id ?>">
			<div class="Xcontent34"><input type="submit" value="立即购买" ></div>
			<div class="Xcontent35"><button id="add_cart" type="button">加入购物车</button></div>
		</ol>
	 </form>
	</ul>
</div>
...
<script type="text/javascript" src="http://localhost:8080/advanced/frontend/web/jQuery-cookie.js"></script>
<script type="text/javascript">
    ...
    $(function(){
    	//获取class为attr_val的值，判断如果和class为attr_name的兄弟元素的值相同的话，默认选中
    	var attrval = $('.attr_val');
		//循环获取属性值，并和默认值对比   	
    	$.each(attrval,function(){
    		var attrname = $(this).siblings('.attr_name').val();
    		if ($(this).val() == attrname) {
	    		$(this).attr('checked',true);
	    	}
    	})  	
    	//获取class为attr_name的值，发送ajax到后台查找当前sku的价格展示
    	ajax();

    })
    //属性值的点击事件
    $('.attr_val').on('click',function(){
    	var attrval = $(this).val();
    	$(this).siblings('.attr_name').val(attrval);
    	$(this).attr('checked',true);
    ...
    function ajax(){
     ...
     $('#add_cart').on('click',function(){
	    	...
	    	//商品属性值拼接
	          ...
	          $.ajax({
	               type:"POST",
	               url:"<?= Url::to(['goods/addcar']) ?>",
	               data:{str:str,number:number,cookie:cookie,g_id:g_id},
	               success:function(res){
	                         if (res ==1) {
                          		alert("此商品库存没有,抱歉");
	                      	  } else if (res ==3){
