	...
	<div class="border_top_cart">
		...
		<div class="container">
			<div class="checkout-box">
				<form  id="checkoutForm" action="index.php?r=order/order2" method="post">
								...
								<div class="clearfix xm-address-list" id="checkoutAddrList">
									...
									<dl class="item" >
										<dt>
											...
											<input type="hidden" value="<?php echo $value['address_id'] ?>"  class="address_id" name="address_id">
											<strong class="itemConsignee"><?php echo $value['user_name']; ?></strong>
										    <span class="morendizhi">默认地址</span><br>
											<span class="itemTag tag">家</span>
										</dt>
										<dd>
											<p class="tel itemTel"><?php echo $value['user_tel']; ?></p>
											<p class="itemRegion"><?php echo $value['address']; ?></p>
							...
							<div class="xm-box">
								...
								<!-- 	<ul id="checkoutPaymentList" class="checkout-option-list clearfix J_optionList">
										<li class="item selected">
											...
											<p>
												<input type="redio">在线支付                               
											</p>
										</li>
									</ul> -->
									<input type="radio" name="p_method" value="1">货到付款 <input type="radio" name="p_method" value="2" checked="checked">在线支付
								</div>
							</div>
							<!-- 支付方式 END-->
							<div class="xm-box">
					...
					<div class="checkout-box-ft">
						...
						<div id="checkoutGoodsList" class="checkout-goods-box">
							<div class="xm-box">
								...
								<div class="box-bd">
                                    ...
                                    <?php foreach ($sku_one as $k => $v) {?> 
										<dl class="checkout-goods-list">
											<dt class="clearfix">
												<span class="col col-1">商品名称</span>	
												<span class="col col-3">购买价格</span>
												<span class="col col-4">购买数量</span>
												<span class="col col-5">小计（元）</span>
											</dt>
											<dd class="item clearfix">
												<div class="item-row">
													<div class="col col-1">
														...
														<div class="g-info">
															<a href="#" target="_blank">
																<?php $sql = "SELECT `g_name` FROM `goods` WHERE g_id=".$v['goods_id'];
		                                                      	$good_name = Yii::$app->db->createCommand($sql)->queryAll();
		                                                      	echo $good_name[0]['g_name'];?>
		                                                        <!--隐藏提交商品名称 -->
		                                                        <input type="hidden" name="g_name[]" value="<?php $sql = "SELECT `g_name` FROM `goods` WHERE g_id=".$v['goods_id'];
		                                                      	$good_name = Yii::$app->db->createCommand($sql)->queryAll();
		                                                      	echo $good_name[0]['g_name'];?>">
															    <!-- 隐藏提交sku_id -->
															    <input type="hidden" name="sku_id[]" value="<?php echo $v['sku_id'] ?>">
															    <!-- 隐藏提交商品的单价 -->
															     <input type="hidden" name="price[]" value="<?php $sql = "SELECT `sku_price` FROM `sku` WHERE sku_id=".$v['sku_id'];
		                                                      	$sku_price = Yii::$app->db->createCommand($sql)->queryAll();
		                                                      	echo $sku_price[0]['sku_price'];?>">
													...
													<div class="col col-3"><?php $sql = "SELECT `sku_price` FROM `sku` WHERE sku_id=".$v['sku_id'];
		                                                      	$sku_price = Yii::$app->db->createCommand($sql)->queryAll();
		                                                      	echo $sku_price[0]['sku_price'];?></div>
													<div class="col col-4">
														<input size="1"  readonly  class="text_box" name="text_box[]" style="text-align: center;" type="text" value="<?php $sql = 'SELECT `num` FROM `shopping` WHERE s_id='.$v['s_id'];
		                                                      	$good_name = Yii::$app->db->createCommand($sql)->queryAll();
		                                                      	echo $good_name[0]['num']; ?>" />
											        </div>
													<div class="col col-5 zj" style="color: red;"><span class="yuan"></span>元  
													</div>
												</div>
											</dd>
										</dl>
									...
									<div class="checkout-count clearfix">
										...
										<div class="checkout-price">
											<ul>
												...
												<li>
													订单总额：<span class="price_qian">元</span>
													          <input type="hidden" value="" id="djzj"  name="djzj">
												</li>
											...
											<p class="checkout-total">·总额:
												<span>
													<strong class="totalPrice"></strong>元
											    </span>
											    <input type="hidden" class="o_price" name="o_price" value="">
						...
						<div class="checkout-confirm">
						    <input type="submit" value ="立即下单" id="btn" width="20px" height="20px">
						</div>
					</div>
			</div>
...
<!-- 引入jquery -->
<script type="text/javascript" src="http://localhost:8080/advanced/frontend/web/jquery-1.8.3.js"></script>
<script type="text/javascript">
	$(function(){
	   ...
	   $(document).ready(function (){ 
		                ...
		                var xin_price =0;
		                 $.each(zj,function(){
							//该商品的数量
							sl = $(this).prev().children(".text_box").val();     
							//该商品的单价              
						 	price = $(this).prev().prev().html();	
						 	zong_price = parseInt(sl*price);
                            $(this).children().html(zong_price);
                            xin_price+=+(zong_price);
                         })
                        zj.parents(".checkout-goods-list").next().children(".checkout-price").children().children().children(".price_qian").html(xin_price);
	                    zj.parents(".checkout-goods-list").next().children(".checkout-price").children(".checkout-total").children().children(".totalPrice").html(xin_price);                  
	                   //商品总价隐藏框赋值
	                    zj.parents(".checkout-goods-list").next().children(".checkout-price").children().children().children("#djzj").val(xin_price);
	                   //订单总价隐藏框赋值
	                      zj.parents(".checkout-goods-list").next().children(".checkout-price").children(".checkout-total").children(".o_price").val(xin_price);       
	           });
	}) 
        
    //默认收货地址选中的点击事件
    $('.morendizhi').on('click',function(){
     //获取地址表里面的主键id
     var address_id = $(this).prev().prev().val();
     //获取该用户的u_id
     var user_id = $(this).prev().prev().prev().val();
       $.ajax({
               type:"POST",
               url:"<?= Url::to(['order/ajaxhou']) ?>",
