						...
						<ul>
							<li>
								<div class="member-minute clearfix">
									<span><?php echo date("Y-m-d H:i:s",$value['o_time']); ?></span>
									<span>订单号：<em><?php echo $value['osn']; ?></em></span>
									...
									<div class="ci1">
										<div class="ci7 clearfix">
											<span class="gr1"><a href="#"><img src="images/shangpinxiangqing/X-1.png" title="" about="" width="60" height="60"></a></span>
											    <span class="gr2">
											    	<a href="">
											    <?php  $sql = "SELECT `g_name` FROM `order_goods` WHERE o_id=".$value['o_id'];
											    $sql_res =  Yii::$app->db->createCommand($sql)->queryOne(); echo  $sql_res['g_name'];?>
											    </a>
											     </span>
											<span class="gr3">X<?php $sql = "SELECT `num` FROM `order_goods` WHERE o_id=".$value['o_id'];
											    $sql_res =  Yii::$app->db->createCommand($sql)->queryOne(); echo  $sql_res['num']; ?>
											    	
											</span>
										</div>
									</div>
									<div class="ci2"><?php $sql = "SELECT `user_name` FROM `address` WHERE address_id=".$value['address_id'];
											    $sql_res =  Yii::$app->db->createCommand($sql)->queryOne(); echo $sql_res['user_name']; ?></div>
									<div class="ci3"><b>￥<?php $sql = "SELECT `goods_price` FROM `order` WHERE o_id=".$value['o_id'];
											    $sql_res =  Yii::$app->db->createCommand($sql)->queryOne(); echo $sql_res['goods_price'];?></b><p>货到付款</p><p class="iphone">手机订单</p></div>
