	...
	<div class="border_top_cart">
		...
		<div class="container">
			<div class="checkout-box">
				<form  id="checkoutForm" action="index.php?r=order/order_do" method="post">
										...
										<dl class="checkout-goods-list">
											...
											<dd class="item clearfix">
												<div class="item-row">
														...
														<div class="g-info">
															<a href="#" target="_blank">
																<?php $sql = "SELECT `g_name` FROM `goods` WHERE g_id=".$v['goods_id'];
		                                                      	$good_name = Yii::$app->db->createCommand($sql)->queryone(); echo  $good_name['g_name'];?>
                                                                <!--隐藏提交商品名称 -->
		                                                      	<input type="hidden" name="g_name" value="<?php echo $good_name['g_name'] ?>">
															    <!-- 隐藏提交sku_id -->
															    <input type="hidden" name="sku_id" value="<?php echo $v['sku_id'] ?>">
															    <!-- 隐藏提交商品的单价 -->
															    <input type="hidden" name="price" value="<?php echo $v['sku_price'] ?>">
															</a> 
													...
													<div class="col col-3"><?php echo $v['sku_price']; ?></div>
													<div class="col col-4">
													<!-- 	<input class="min" name="" type="button" value="-" /> -->
														<input size="1"  readonly  class="text_box" name="text_box" style="text-align: center;" type="text" value="<?php echo $num ?>" />
													...
													<div class="col col-5 zj" style="color: red;"><span class="yuan"></span>元
														...
														<!-- 隐藏提交购买商品数量 -->
													     <input type="hidden" name="num" class="num" value="">
													</div>
												</div>
											</dd>
										</dl>
...
<script type="text/javascript">
		                 ...
		                 $.each(zj,function(){
							...
							//该商品的单价              
						 	price += $(this).prev().prev().html();	
                                     $(this).children().html(sl*price);
                                     $(this).parents(".checkout-goods-list").next().children(".checkout-price").children().children().children(".price_qian").html(sl*price);
                                     $(this).parents(".checkout-goods-list").next().children(".checkout-price").children(".checkout-total").children().children(".totalPrice").html(sl*price);
                                     //隐藏替换小计商品总价
                                     $(this).parents(".checkout-goods-list").next().children(".checkout-price").children(".checkout-total").children(".o_price").val(sl*price);
                                     //隐藏替换总计商品价格
                                     $(this).children().val(sl*price);
                                     //隐藏显示购买数量
                                     $(this).children(".num").val(sl); 
                         })
    ...
    $('.morendizhi').on('click',function(){
     ...
     var user_id = $(this).prev().prev().prev().val();
       $.ajax({
               type:"POST",
               url:"http://127.0.0.1/ShoppingMall/advanced/frontend/web/index.php?r=order/ajaxhou",
