   ...
   <tbody>
    ...
    <tr>
      ...
      <td><?php echo $value['attr_name']; ?></td>
       ...
       <td class="td-manage">
        <input type="hidden" id="attr_id" value="<?php echo $value['attr_id']; ?>" />
        
        <a title="删除" href="javascript:;"   class="del" class="btn btn-xs btn-delete" >删除</a>
        <a title="修改" href="javascript:;"   class="update" class="btn btn-xs btn-delete" >修改</a>
...
$('.del').click(function() {
  ...
  $(this).parent().parent().remove()
    $.ajax({
      url:'<?= Url::to(['product/attr_del']);?>',
      data:{'id':id},
      type:'post',
      success:function(msg){
        if(msg == 1){
          window.location.href = '<?= Url::to(['product/attr_list']);?>';
        }
      }
    });
}) 
